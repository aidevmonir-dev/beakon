 ...
 *
 * Behaviour:
 *   - Authenticated visitors are redirected straight into the app
 *     (/dashboard if they have an org, /setup otherwise) — they don't
 *     need to see the entry screen again.
 *   - Anonymous visitors see the three doors. A subtle
 *     "Learn more about Beakon" link sends curious visitors to the
 *     marketing landing at /about.
 *
 * The marketing landing page that used to live at `/` was moved to
 * `/about` so the doc-prescribed entry can occupy the root URL.
 */
...
export default function EntryPage() {
  ...
  if (redirecting) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-canvas-50">
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-canvas-50">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-5xl flex-col items-center justify-center px-6 py-16">
        ...
        <Link
          href="/about"
          className="mt-3 text-[12px] text-gray-400 hover:text-brand-700 hover:underline"
        >
          Learn more about Beakon →
        </Link>
      </div>
    </main>
  );
